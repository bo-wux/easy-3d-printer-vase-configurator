...
const createExtrusionTube = (path, segments, lineWidth, lineHeight, rectangular = false, cornerRadius = 0) => {
  ...
  if (rectangular) {
    ...
    if (effectiveRadius > 0.01) {
      ...
      // Scherpe hoeken - gewone rechthoek
  ...
  const printLines = useMemo(() => {
    ...
    if (spiralMode) {
      ...
      for (let i = 0; i <= totalPoints; i++) {
        ...
        spiralPoints.push(new THREE.Vector3(p.x, y, p.z));
      }
    ...
    return lines;
  }, [params, previewLineWidth, spiralMode, rectangularLine, cornerRadius, actualLayerHeight, previewLayers]);
